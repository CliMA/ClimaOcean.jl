...
[`Metadata`](@ref) is an abstraction that _represents_ data, but does
not embody it. Unlike [`Field](@ref), which points to an array occupying space in memory,
...
When `Metadata` represents just one date, we call it "`Metadatum`".
...
This looks odd, but less so if we download bathymetry (also using `Metadata` under the hood)
to create a temperature field with a land mask,
...
- the dataset (such as [`EN4Monthly`](@ref), [`ECCO2Daily`](@ref), or [`GLORYSMonthly`](@ref));
...
- the on-disk `dir`ectory where the dataset should be cached.
